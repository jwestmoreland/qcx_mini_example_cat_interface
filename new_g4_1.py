# qcx_ultimate_gui_with_polling.py
# Polling interval adjustable 0.1 to 10 seconds + all previous features
...
class QCXUltimateGUI:
    def __init__(self, root):
        ...
        self.poll_interval = 1000  # Default 1 second
        ...
        # Polling interval
        tk.Label(top_frame, text="Poll (s):", fg="cyan", bg="#1a1a1a", font=("Arial", 12)).pack(side=tk.LEFT, padx=10)
        self.poll_var = tk.DoubleVar(value=1.0)
        poll_spin = tk.Spinbox(top_frame, from_=0.1, to=10.0, increment=0.1, textvariable=self.poll_var, width=5, command=self.update_poll_interval)
        poll_spin.pack(side=tk.LEFT, padx=5)
    ...
    def update_poll_interval(self):
        # Called when spinbox changes
        interval = self.poll_var.get()
        self.poll_interval = int(interval * 1000)  # Convert to ms
        # Restart polling with new interval
        self.root.after_cancel(self.poll_id) if hasattr(self, 'poll_id') else None
        self.poll_status()
...
#                    self.tb_text.see(tk.END)
            ...
            if tb_resp.startswith('TB') and len(tb_resp) > 4:  # More than just "TB000;"
                decoded = tb_resp[2:].strip()
                if decoded and decoded != "000" and not decoded.isdigit():  # Filter junk
                    self.tb_text.insert(tk.END, decoded + " ")
                    self.tb_text.see(tk.END)
    ...
    # ... (all other methods unchanged — set_vfo, toggle_split, band_change, set_freq, rit, speed, messages, TX safety, debug console, etc.) ...
    ...
    def wheel_tune(self, event):
        ...
        # Step size
        if ctrl:
            delta = step * 1000  # 1 kHz
        elif shift:
            delta = step * 100   # 100 Hz
        else:
            delta = step * 10    # 10 Hz
