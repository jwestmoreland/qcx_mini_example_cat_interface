# qcx_cw_decoder.py - v12 (all nonlocal fixed with mutable lists)
# Current_symbol also made mutable
...
def open_cw_decoder(main_app):
    ...
    current_symbol_state = [""]  # mutable string
        ...
        while decoding_state[0]:
            try:
                ...
                if key_down != key_state_state[0]:
                    ...
                    if key_down:
                        win.after(0, lambda: symbol_label.config(text=current_symbol_local + " [on]"))
                        ...
                        if element_times:
                            ...
                            win.after(0, lambda: symbol_label.config(text=current_symbol_local))
                            if len(current_symbol_local) > 5:
                                win.after(0, lambda: symbol_label.config(text=current_symbol_local + " [decoding...]"))

                    key_state_state[0] = key_down
                    last_transition_state[0] = now
                    last_char_time_state[0] = now

                if not key_down and current_symbol_local:
                    ...
                    if silence_time > char_space_threshold:
                        current_symbol_state[0] = current_symbol_local  # copy to shared
                        decode_char()
                        current_symbol_local = ""

                if not key_down and (now - last_transition_state[0]) > 0.5:
                    current_symbol_state[0] = current_symbol_local
                    decode_char()
                    current_symbol_local = ""
