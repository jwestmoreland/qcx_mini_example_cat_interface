...
# - More forgiving inter-character timing (now * 2.5)
    ...
    def audio_decoder():
        ...
        while decoding:
            try:
                ...
                dynamic_thresh = avg_noise * 4  # adjustable - lower for more sensitivity
                ...
                # More forgiving inter-character timing (was * 3, now * 2.5)
                if not key_down and current_symbol:
#                    char_space_threshold = np.mean(element_times) * 2.5 if element_times else 0.25
                    char_space_threshold = np.mean(element_times) * 2.0 if element_times else 0.25
                    if (now - last_char_time) > char_space_threshold:
                        print(f"DEBUG: Inter-character space detected ({(now - last_char_time):.3f}s > {char_space_threshold:.3f}s) - decoding: {current_symbol}")
                        decode_char()
                        last_char_time = now
