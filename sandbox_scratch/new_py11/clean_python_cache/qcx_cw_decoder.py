...
# - Very forgiving inter-character timing (now * 1.8 + min 0.1s)
    ...
    def audio_decoder():
        ...
        while decoding:
            try:
                ...
                dynamic_thresh = avg_noise * 4  # adjustable
                ...
                # Very forgiving inter-character timing (was * 2.5, now * 1.8 + min 0.1s)
                if not key_down and current_symbol:
                    char_space_threshold = max(0.1, np.mean(element_times) * 2.0) if element_times else 0.1
                    silence_time = now - last_char_time
                    if silence_time > char_space_threshold:
                        print(f"DEBUG: Inter-character space detected ({silence_time:.3f}s > {char_space_threshold:.3f}s) - decoding: {current_symbol}")
                        decode_char()
                        last_char_time = now
...
# char_space_threshold = max(0.1, np.mean(element_times) * 1.8) if element_times else 0.1