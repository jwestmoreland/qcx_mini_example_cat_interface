...
# - Very forgiving inter-character timing (* 1.5 + min 0.08s)
# - Max symbol length 10 to prevent runaway
# - Always insert char or '?' on decode
# - Improved dot/dash classification (* 2.5 ratio)
# - Extra debug for added elements
...
def open_cw_decoder(main_app):
    ...
    def decode_char():
        ...
        if current_symbol:
            ...
            # Always insert char or '?' on failure
            text_area.insert(tk.END, char if char != '?' else '?')
                ...
                if key_down != key_state:
                    ...
                    if key_down:
                        ...
                        if element_times:
                            ...
                            # Improved classification: dit if < 2.5 * avg_dot (standard ratio ~3:1)
                            if duration < avg_dot * 2.5:
                                current_symbol += "."
                                timing_label.config(text=f"{duration:.3f}s [dot]")
#                                print(f"DEBUG: Added dot (duration: {duration:.3f}s, avg_dot: {avg_dot:.3f}s)")
                            else:
                                current_symbol += "-"
                                timing_label.config(text=f"{duration:.3f}s [dash]")
#                                print(f"DEBUG: Added dash (duration: {duration:.3f}s, avg_dot: {avg_dot:.3f}s)")
                ...
                # Extremely forgiving inter-character timing
                if not key_down and current_symbol:
                    char_space_threshold = max(0.05, np.mean(element_times) * 1.5) if element_times else 0.05
##                    char_space_threshold = max(0.05, np.mean(element_times) * 1.2) if element_times else 0.05
                    ...
                    if silence_time > char_space_threshold:
                        ...
                        # NEW: Limit length to prevent runaway
                        if len(decoded_symbol) > 10:
