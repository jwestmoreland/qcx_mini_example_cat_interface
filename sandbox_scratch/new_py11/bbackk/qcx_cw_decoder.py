...
#            return
    try:
        name = device_var.get()
        idx = next(i for n, i in devices if n == name)
        print(f"DEBUG: Attempting to open audio device: {name} (index {idx})")
        stream = audio.open(format=pyaudio.paInt16, channels=1, rate=rate,
                        input=True, input_device_index=idx, frames_per_buffer=chunk)
        print("DEBUG: Audio stream OPENED successfully")
    except Exception as e:
        print(f"DEBUG: CRITICAL - Failed to open audio stream: {str(e)}")
        messagebox.showerror("Audio Error", f"Cannot open audio input:\n{str(e)}\n\nCheck device selection and permissions.")
        return
