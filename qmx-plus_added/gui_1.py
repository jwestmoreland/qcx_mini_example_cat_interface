...
class QCXUltimateGUI:
    def __init__(self, root):
        ...
        # Updated: Added QMX+ option
        ttk.Combobox(top_frame, textvariable=self.device_var, values=["QCX", "QMX", "QMX+"], width=6).pack(side=tk.LEFT, padx=5)
        tk.Label(top_frame, text="Variant:", fg="cyan", bg="#1a1a1a", font=("Arial", 12)).pack(side=tk.LEFT, padx=5)
        ttk.Combobox(top_frame, textvariable=self.variant_var, values=["Low", "Mid", "High"], width=5).pack(side=tk.LEFT, padx=5)
    ...
    def scan_loop(self):
        ...
        if width_str == "Full Band" and band == "40m":
            # Hard-coded full 40m band: 7.000 to 7.300 MHz
            center_freq = 7.150  # Exact center
            width_khz = 150      # ±150 kHz = 300 kHz total span
    ...
    # All remaining methods unchanged (set_vfo, toggle_split, band_change, etc.)
    # (Identical to previous complete version)
    ...
    def update_bands(self):
        ...
        if self.device_var.get() in ["QMX", "QMX+"]:
            if variant == "Low":
