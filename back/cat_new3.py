# qcx_ultimate_gui_simple_vfo_split.py
# VFO A/B dropdown + separate SPLIT button
...
class QCXUltimateGUI:
    def __init__(self, root):
        ...
        self.split_banner = tk.Label(vfo_frame, text="", fg="white", bg="#880000", font=("Arial", 20, "bold"))
        ...
        tk.Label(cust_frame, text="VFO:", fg="white", bg="#1a1a1a").pack(side=tk.LEFT, padx=5)
        self.vfo_var = tk.StringVar(value="A")
        vfo_radio_a = tk.Radiobutton(cust_frame, text="A", variable=self.vfo_var, value="A", bg="#1a1a1a", fg="white", selectcolor="#1a1a1a")
        vfo_radio_a.pack(side=tk.LEFT)
        vfo_radio_b = tk.Radiobutton(cust_frame, text="B", variable=self.vfo_var, value="B", bg="#1a1a1a", fg="white", selectcolor="#1a1a1a")
        vfo_radio_b.pack(side=tk.LEFT, padx=5)
        tk.Button(cust_frame, text="SPLIT", command=self.toggle_split, bg="#ff8800", fg="black").pack(side=tk.LEFT, padx=10)
    ...
    def poll_status(self):
        if self.ser:
            ...
            if ft_resp == 'FT0':
                self.mode_label.config(text="Mode: VFO A (TX & RX on A)")
            elif ft_resp == 'FT1':
                self.mode_label.config(text="Mode: VFO B (TX & RX on B)")
            elif ft_resp == 'FT2':
                self.mode_label.config(text="Mode: SPLIT (RX on A, TX on B)")
            else:
            ...
            # Practice Mode (manual only)
            self.practice_label.config(text="Practice Mode: Set in radio menu 8.7")
    ...
    def toggle_split(self):
        ...
        if current == 'FT2':
            self.send_cmd('FT0')  # Turn off split
        else:
            self.send_cmd('FT2')  # Turn on split
